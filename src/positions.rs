...
impl Coding {
    ...
    fn try_from_genomic(
        ...
        let position = genomic.position();
        let mut tmp = exons.to_vec();
        tmp.extend([start, stop]);
        #[cfg(not(feature = "parallel"))]
        tmp.sort();
        #[cfg(feature = "parallel")]
        tmp.par_sort();
        let not_coding_limit: Vec<std::ops::Range<u64>> =
            tmp.chunks(2).map(|x| (x[0])..(x[1])).collect();

        let mut tmp = exons[1..].to_vec();
        dbg!(&tmp);
        tmp.pop();
        dbg!(&tmp);
        tmp.extend([start, stop]);
        ...
        #[cfg(feature = "parallel")]
        tmp.par_sort();
        let coding_limit: Vec<std::ops::Range<u64>> = tmp.chunks(2).map(|x| x[0]..x[1]).collect();
        ...
        match not_coding_limit.iter().position(|x| x.contains(position)) {
            Some(index) => {
                ...
                if index == 0 {
                    Some(Coding::new(
                        1,
                        *position as i64 - coding_limit[0].start as i64,
                    ))
                } else {
                    Some(Coding::new(
                        coding_limit[..index].iter().map(|x| x.end - x.start).sum(),
                        *position as i64 - coding_limit[index - 1].end as i64,
                    ))
                }
            }
            None => coding_limit
...
impl Transcript {
    ...
    pub fn try_from_coding(coding: Coding) -> Self {
...
impl Protein {
    ...
    fn try_from_transcript(transcript: Transcript) -> Option<Self> {
        if transcript.position.is_negative() {
            ...
            Some(Protein {
                position: (transcript.position as u64)
                    .saturating_div(3)
                    .saturating_add(1),
            })
        }
...
mod tests {
    ...
    fn transcript() -> error::Result<()> {
        ...
        let transcript = Transcript::try_from_coding(coding);
